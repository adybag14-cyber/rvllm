//! rvllm-v3 throughput benchmark.
//!
//! Same measurement methodology as `rvllm-v2-bench` (pipelined step loop,
//! 3 iters, tok/s = batch * output_len / elapsed). Driven through the
//! v3 `Engine` so the test exercises the v3 API surface and typed errors.
...
use rvllm_v2::integration::V2EngineConfig;
use rvllm_v3::Engine;
...
#[derive(Parser)]
#[command(name = "rvllm-v3-bench", about = "v3 engine throughput benchmark")]
struct Cli {
    #[arg(long)]
    model: String,
...
fn print_revision() {
    ...
    match rev {
        Ok(sha) => eprintln!("rvllm v3 bench, revision: {}", sha.trim()),
        Err(_) => eprintln!("rvllm v3 bench, revision: UNKNOWN"),
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    if batch_sizes.len() > 1 || iters > 1 {
        eprintln!("rvllm v3 bench -- direct engine, no HTTP");
        eprintln!(
    ...
    // Construct engine once; max_num_seqs covers all batch sizes.
    ...
    let mut engine = match Engine::new(config) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("rvllm v3: engine init failed: {e}");
            return Err(anyhow::anyhow!("{e}"));
        }
    };

    for &batch in &batch_sizes {
        // Warmup with identical output_len to prime the same kernel buckets.
        ...
        for i in 0..batch {
            ...
            engine
                .add_request(prompt, warmup_params.clone())
                .map_err(|e| anyhow::anyhow!("{e}"))?;
        }
        while engine.has_pending_work() {
            let _ = engine.step().map_err(|e| anyhow::anyhow!("{e}"))?;
        }
        let _ = engine.flush().map_err(|e| anyhow::anyhow!("{e}"))?;
        ...
        for _ in 0..iters {
            ...
            let final_outputs = engine.flush().map_err(|e| anyhow::anyhow!("{e}"))?;
