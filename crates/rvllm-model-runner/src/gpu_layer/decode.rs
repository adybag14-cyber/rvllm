...
use cudarc::driver::{
    CudaSlice, CudaStream, CudaViewMut, DevicePtr, DevicePtrMut, DeviceSlice, LaunchConfig,
    PushKernelArg,
};
use half::f16;
use tracing::info;
...
impl GpuTransformerLayer {
    ...
    pub(crate) fn forward_fp8_decode(
        ...
        let gu_fp8 = weights.fused_gate_up_fp8.ok_or_else(|| {
            LLMError::GpuError("Fp8Decode requires fused_gate_up_fp8 weights".into())
        })?;
        let down_fp8 = weights
            .down_proj_fp8
        ...
        let cast_fn = self
            ...
            .map_err(|e| {
                LLMError::GpuError(format!("Fp8Decode requires cast_f16_to_fp8 kernel: {e}"))
            })?;

        let fp8_scratch_ptr = input.fp8_input_scratch_ptr;
        if fp8_scratch_ptr == 0 {
            return Err(LLMError::GpuError(
                "Fp8Decode requires non-zero fp8_input_scratch_ptr".into(),
            ));
        }
        ...
        let fused_act =
            Self::fused_silu_mul_f16_split(&self.stream, &self.loader, &gate_up, intermediate)?;
    ...
    pub(crate) fn forward_fused_decode(
        ...
        if profile {
            info!("PROFILE L0 path=f16_fused");
        }
        prof!("enter");

        // --- Steps 1-2: Fused add+norm+QKV GEMV ---
        let fused_qkv_w = weights
            .fused_qkv
            .expect("FusedDecode requires fused_qkv weight");
        let (mut qkv, residual_ref, bias_fused) = if let Some(prev_mlp) = prev_mlp_out {
            self.fused_add_norm_qkv(
                input.hidden_states,
                prev_mlp,
                weights,
                fused_qkv_w,
            ...
            self.fused_norm_qkv(input.hidden_states, weights, fused_qkv_w, hidden, qkv_dim)?
        };
    ...
    fn fused_add_norm_qkv(
        ...
        if let (Some(fp8_w), Some(fp8_s), Ok(ref fk)) = (
            ...
            self.loader
                .get_func("gemv_fp8", "fused_add_norm_fp8_gemv_kernel"),
        ) {
            // Reinterpret u8 slice as f16 for the launch builder (kernel handles FP8 internally)
            let fp8_as_f16 =
                unsafe { std::mem::transmute::<&CudaSlice<u8>, &CudaSlice<f16>>(fp8_w) };
            return self.launch_fused_gemv_2out(
                fk,
                hidden_states,
                prev_mlp,
                norm_w,
        ...
        // f16 plain variant
        if let Ok(ref fk) = self
            .loader
            .get_func("fused_add_norm_qkv_gemv", "fused_cute_add_norm_qkv_gemv")
        {
            return self.launch_fused_gemv_2out(
                fk,
                hidden_states,
        ...
        Err(LLMError::GpuError(
            "No fused add+norm+QKV GEMV kernel available. \
             Required: fused_cute_add_norm_qkv_gemv or bias/fp8 variant."
                .into(),
        ))
    }
    ...
    fn fused_norm_qkv(
        ...
        if let (Some(qkv_bias), Ok(ref fk)) = (
            weights.qkv_bias,
            self.loader
                .get_func("fused_add_norm_qkv_gemv", "fused_cute_norm_qkv_bias_gemv"),
        ) {
        ...
        if let Ok(ref fk) = self
            .loader
            .get_func("fused_add_norm_qkv_gemv", "fused_cute_norm_qkv_gemv")
        {
            let mut qkv_out = unsafe { self.stream.alloc::<f16>(qkv_dim) }
                .map_err(|e| LLMError::GpuError(format!("fused qkv alloc: {e}")))?;
            let smem = (hidden * 4 + 8 * 4) as u32;
        ...
        Err(LLMError::GpuError(
            "No fused norm+QKV GEMV kernel available (first layer). \
             Required: fused_cute_norm_qkv_gemv or bias variant."
                .into(),
        ))
    }
    ...
    fn fused_oproj_mlp(
        ...
        let fused_gate_up_w = weights
            .fused_gate_up
            .expect("FusedDecode requires fused_gate_up weight");
        let gate_up_dim = intermediate * 2;
        ...
        if let (true, Some(o_fp8), Some(_o_sc), Some(gu_fp8), Some(_gu_sc), Ok(ref mk)) = (
            oproj_fp8_ok,
            weights.o_proj_fp8,
            weights.o_proj_fp8_scale,
            weights.fused_gate_up_fp8,
            weights.fused_gate_up_fp8_scale,
            self.loader.get_func(
            ...
            unsafe {
                self.stream
                    ...
                    .launch(LaunchConfig {
                        grid_dim: ((gate_up_dim as u32 + rpb - 1) / rpb, 1, 1),
                        block_dim: (256, 1, 1),
                        shared_mem_bytes: smem,
                    })
        ...
        let attn_proj = Self::hgemm_dispatch(
            &self.stream,
            blas,
            lt,
            attn_out,
        ...
        let fk = self
            ...
            .map_err(|e| {
                LLMError::GpuError(format!(
                    "Required fused_cute_add_norm_gateup_gemv kernel missing: {e}"
                ))
            })?;
        let mut gate_up_out = unsafe { self.stream.alloc::<f16>(gate_up_dim) }
    ...
    pub(crate) fn forward_cublas_decode(
        ...
        let fused_qkv_w = weights
            .fused_qkv
            .expect("CublasGemvDecode requires fused_qkv weight");
        let fused_gate_up_w = weights
            .fused_gate_up
            .expect("CublasGemvDecode requires fused_gate_up weight");
        ...
        let mut qkv = Self::hgemm_dispatch(
            &self.stream,
            blas,
            lt,
            &normed,
        ...
        let gate_up = Self::hgemm_dispatch(
            &self.stream,
            blas,
            lt,
            &normed2,
        ...
        let mlp_out = Self::hgemm_dispatch(
            &self.stream,
            blas,
            lt,
            &activated,
    ...
    pub(crate) fn forward_persistent_decode(
        ...
        let fused_qkv = weights.fused_qkv.ok_or_else(|| {
            LLMError::GpuError("PersistentDecode requires fused_qkv weight".into())
        })?;
        let fused_gate_up = weights.fused_gate_up.ok_or_else(|| {
            LLMError::GpuError("PersistentDecode requires fused_gate_up weight".into())
        })?;
        ...
        if smem > 49152 {
            let cu_func = self
                .loader
                .get_cubin_func("persistent_layer_decode", "persistent_layer_decode_f16")?;
            unsafe {
        ...
        let mlp_out = unsafe { self.stream.alloc::<f16>(hidden) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        let residual_out = unsafe { self.stream.alloc::<f16>(hidden) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        let qkv_scratch = unsafe { self.stream.alloc::<f16>(qkv_dim) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        let attn_scratch = unsafe { self.stream.alloc::<f16>(q_dim) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        let oproj_scratch = unsafe { self.stream.alloc::<f16>(hidden) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        let gateup_scratch = unsafe { self.stream.alloc::<f16>(gate_up_dim) }
            .map_err(|e| LLMError::GpuError(format!("pld alloc: {e}")))?;
        ...
        let null: u64 = 0;
        let p_prev_mlp = prev_mlp_out
            .map(|s| DevicePtr::device_ptr(s, &self.stream).0)
            .unwrap_or(null);
    ...
    fn fused_silu_down(
        ...
        if let (Some(d_fp8), Some(d_sc), Ok(ref sk)) = (
            ...
            weights.down_proj_fp8_scale,
            self.loader
                .get_func("fused_silu_down_gemv", "fused_cute_silu_down_fp8_gemv"),
        ) {
            ...
            unsafe {
                self.stream
                    ...
                    .arg(&(intermediate as i32))
                    .launch(LaunchConfig {
                        grid_dim: (((hidden as u32) + 7) / 8, 1, 1),
                        block_dim: (256, 1, 1),
                        shared_mem_bytes: (8 * 4) as u32,
                    })
        ...
        let sk = self
            .loader
            .get_func("fused_silu_down_gemv", "fused_cute_silu_down_gemv")
            .map_err(|e| {
                LLMError::GpuError(format!(
                    "Required fused_cute_silu_down_gemv kernel missing: {e}"
                ))
            })?;
        let mut down_out = unsafe { self.stream.alloc::<f16>(hidden) }
