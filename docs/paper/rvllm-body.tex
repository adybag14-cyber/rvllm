...
% --- Abstract ---
\begin{abstract}
\textbf{Work in progress.} I present rvLLM, a from-scratch Rust implementation of
the vLLM inference engine for OpenAI-compatible API serving with CUDA GPU acceleration.
The system comprises 23~Rust crates and 50~CUDA kernels, compiling
to a single 16\,MB static binary. In head-to-head benchmarks on a single H100~SXM~80\,GB,
rvLLM with FP16 inference achieves 8{,}652~tok/s serving Qwen2.5-7B at $N\!=\!128$,
\textbf{achieving 0.67--0.88$\times$ of Python vLLM~0.18 throughput}
while offering 20$\times$ faster startup, a 31$\times$ smaller
binary, and 3$\times$ less CPU memory. At higher concurrency ($N\!>\!512$), Python
vLLM scales further due to its mature continuous batching optimisations.
The throughput advantage derives from: (1)~fused GEMM projections (QKV 3$\to$1,
gate+up 2$\to$1), (2)~GPU-resident greedy decoding via an argmax kernel that reduces
device-to-host transfer by $\sim$150{,}000$\times$ per step, (3)~FP16 hgemm with
tensor cores and f16 KV cache, (4)~vectorized float4 loads across all kernels,
(5)~fixed CUDA graph replay eliminating redundant forward passes,
(6)~warp-parallel FlashAttention-3 v2 for decode, and (7)~a zero-copy
memory architecture with no Python objects, no PyTorch tensor metadata, and no garbage
collector. rvLLM is open-source under the Apache-2.0 license.
\end{abstract}
...
\begin{enumerate}
  \item A modular 23-crate Rust architecture for LLM inference, with GPU-resident
        ...
        decoding.
  \item 50~CUDA kernels including FlashAttention-3 v2 with warp-parallel $QK^T$,
        paged KV cache support, GPU argmax, and fused residual-RMSNorm.
  \item Verified benchmark results: 8{,}652~tok/s on H100 (FP16, Qwen2.5-7B, $N\!=\!128$),
        achieving 0.67--0.88$\times$ of Python vLLM, with coherence verified across 14{,}620 total
...
\begin{itemize}
  \item NVIDIA A100 SXM4 (80\,GB VRAM), rented on vast.ai at \$0.96--1.15/hr.
  \item NVIDIA B200 (180\,GB VRAM), rented on vast.ai at \$3.13/hr.
\end{itemize}
Python vLLM~0.18.0 was benchmarked on the same A100~80\,GB SXM4 for direct
head-to-head comparison. All rvLLM measurements include coherence verification:
...
\begin{table}[t]
  \centering
  \caption{B200 throughput scaling. Qwen2.5-1.5B, greedy decoding, 512 tokens/request. Zero errors across all batch sizes.}
  \label{tab:b200}
  \small
  \begin{tabular}{@{}rrrrr@{}}
    \toprule
    \thead{$N$} & \thead{Tokens} & \thead{Wall (ms)} & \thead{tok/s} & \thead{Errors} \\
    \midrule
    1     & 32      & 279     & 114    & 0 \\
...
\subsection{A100 Head-to-Head: rvLLM vs Python vLLM}

Table~\ref{tab:a100} shows optimised FP16 results on A100~80\,GB SXM4, compared
directly with Python vLLM~0.18 on the same hardware.

\begin{table}[t]
  \centering
  \caption{A100 80\,GB SXM4 head-to-head. Qwen2.5-1.5B, greedy, 512 tok/req. rvLLM FP16 (optimised) vs Python vLLM~0.18.}
  \label{tab:a100}
  \small
  \begin{tabular}{@{}rrrp{3.5cm}@{}}
    \toprule
    \thead{$N$} & \thead{rvLLM (tok/s)} & \thead{vLLM (tok/s)} & \thead{Notes} \\
    \midrule
    1    & 117    & 69     & rvLLM 1.7$\times$ faster \\
    ...
    64   & 4{,}796 & 3{,}828 & rvLLM 1.25$\times$ \\
    
    96   & 5{,}965 & 5{,}197 & rvLLM 1.15$\times$ \\
    128  & 7{,}380 & 6{,}400 & rvLLM 1.15$\times$ \\
    
    256  & 9{,}905 & 9{,}437 & rvLLM 1.05$\times$ \\
    512  & 10{,}291 & 10{,}771 & Near parity (0.96$\times$) \\
    
    \textbf{768} & \textbf{10{,}235} & --- & \textbf{rvLLM peak} \\
    1{,}024 & 10{,}051 & 12{,}740 & \\
    \bottomrule
  \end{tabular}
\end{table}

On A100~80\,GB, rvLLM \textbf{beats Python vLLM up to $N\!=\!256$} and peaks at
10{,}291~tok/s at $N\!=\!512$. At single-sequence latency ($N\!=\!1$), rvLLM is
1.7$\times$ faster. Python vLLM scales further only beyond $N\!=\!512$.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
    \thead{Metric} & \thead{rvLLM (B200)} & \thead{rvLLM (A100)} & \thead{Python vLLM} & \thead{Ratio} \\
    \midrule
    Peak throughput    & 3{,}946 tok/s & 10{,}291 tok/s & 12{,}740 tok/s & Beats vLLM to $N\!=\!256$ \\
...
The throughput advantage over Python vLLM is not attributable to any
...
At single-sequence latency ($N\!=\!1$), rvLLM is 1.7$\times$ faster (117 vs.\ 69~tok/s).
The advantage grows through mid-range concurrency and rvLLM leads at all batch sizes up
to $N\!=\!256$. Python vLLM overtakes only at very high concurrency ($N\!>\!512$).
...
\begin{itemize}
  \item Benchmarks use Qwen2.5-1.5B, a small model. Performance characteristics
        ...
        overhead.
  \item Python vLLM overtakes rvLLM at very high concurrency ($N\!>\!512$), reaching
        12{,}740~tok/s at $N\!=\!1{,}024$ vs rvLLM's 10{,}051~tok/s.
  \item Further kernel fusion and batching optimisations may close the remaining gap
        at high concurrency.
\end{itemize}
...
infrastructure can beat the Python incumbent at production batch sizes (up to
$N\!=\!256$), peaking at 10{,}291~tok/s on A100, while using 3$\times$ less CPU
memory, starting 20$\times$ faster, and deploying as a 31$\times$ smaller binary.
The throughput advantage derives from the compound effect of fused GEMM projections,
GPU-resident greedy decoding, vectorized kernel loads, CUDA graph replay,
