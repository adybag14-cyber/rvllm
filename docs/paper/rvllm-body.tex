...
80\,GB running Qwen2.5-7B in FP8~E4M3 with greedy decoding and a single
CUDA-graph replay per step, rvLLM~v3 reaches \textbf{22{,}559 tokens/second
at $N\!=\!128$} (decode + LM head + argmax, 30 iterations, 5 warmup). The
earlier v2 stack peaked at 19{,}287 tok/s at the same batch on the same GPU
(SHA \texttt{eb9e247fd}, April~16, 2026) and is kept in-tree as an archive.

v3 ships as a FlashAttention-3 SM90 paged-KV decode kernel (built from the
Hopper source to \texttt{libfa3\_kernels.so}), five FP8 linears driven by
\texttt{cublasLtMatmul} with fused bias and residual epilogues, a set of
custom fused PTX kernels (add+RMSNorm+quantize, RoPE~+~KV-cache write,
SiLU$\cdot$mul+quantize, per-token quantize, argmax), and a graph-capture
runtime that records \textbf{339 kernel launches per decode step into a
single \texttt{cuGraphLaunch}}. Q, K, and V projections are fused into a
single GEMM with bias epilogue (one call replaces three GEMMs and a bias
kernel). No megakernels: each launch is a recognizable composite.
...
This paper documents rvLLM~v3, which achieves 22{,}559~tok/s at
$N\!=\!128$ on a single H100. The previous v2 stack (archived at commit
\texttt{eb9e247fd}, April~16, 2026) reached 19{,}287~tok/s; v3 is 1.17$\times$
faster on the same GPU with the same model. v3's primary levers are: (i)
...
describes the v3 session (551\,\ensuremath{\rightarrow}\,22{,}559 tok/s).
...
\subsection{v3 throughput by batch size}

\begin{tabular}{rrr}
\toprule
$\boldsymbol{N}$ & \textbf{tok/s} & \textbf{ms/step} \\
\midrule
32  & 6{,}644 & 4.82 \\
64  & 12{,}615 & 5.07 \\
128 & \textbf{22{,}559} & \textbf{5.67} \\
\bottomrule
\end{tabular}

\subsection{v3 vs v2 archive at $N\!=\!128$}

\begin{tabular}{lrr}
\toprule
\textbf{Stack} & \textbf{tok/s} & \textbf{ratio} \\
\midrule
rvllm-v2 (\texttt{eb9e247fd}, April 16 2026) & 19{,}287 & 1.00$\times$ \\
\textbf{rvllm-v3} & \textbf{22{,}559} & \textbf{1.17}$\boldsymbol\times$ \\
\bottomrule
\end{tabular}

v3's gain comes from four structural changes: (i) fused Q$\|$K$\|$V GEMM
(one launch, $N\!=\!4608$) versus v2's three separate Q/K/V GEMMs; (ii)
\texttt{cublasLtMatmul} replacing hand-picked CUTLASS variants on all five
linears; (iii) QKV bias fused into the GEMM epilogue via
\texttt{CUBLASLT\_EPILOGUE\_BIAS}; (iv) O and down residual-add fused into
the GEMM via \texttt{beta}=1 against the residual tensor. Each change
removes a kernel launch without introducing a megakernel.
...
\textbf{+ all 5 linears on cuBLASLt}          & \textbf{22{,}559} & \textbf{+28\%} \\
\bottomrule
\end{tabular}
...
\textbf{22{,}559~tok/s} serving Qwen2.5-7B in FP8 on a single H100, with
