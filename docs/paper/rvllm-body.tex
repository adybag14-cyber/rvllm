...
\begin{center}
  ...
  \vspace{0.3em}
  {\normalsize San Francisco, April 7, 2026\par}
\end{center}
\vspace{1em}
...
132.7~tok/s at $N\!=\!1$ (\textbf{0.79$\times$ vLLM}),
8{,}503.4~tok/s at $N\!=\!64$ (\textbf{0.91$\times$ vLLM}), and
13{,}718.1~tok/s at $N\!=\!128$ (\textbf{0.82$\times$ vLLM}). The most important
changes in this revision are architectural, tooling-heavy, and corrective rather than cosmetic:
batch-1 normal decode now defaults to the reusable batched scratch path, batched
execution uses an explicit hybrid GEMM policy with QKV/O/down on cuBLAS or
cublasLt, and the dominant Hopper FFN seam can now be profiled and reasoned about
correctly. The earlier fast `89f` CUTLASS gate-aux path is no longer treated as
a valid benchmark win because that branch skipped the FFN down-projection. rvLLM is open-source under the
Apache-2.0 license.
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rrrr@{}}
    ...
    \midrule
    1   & 167.5 & 132.7 & 0.79$\times$ \\
    32  & 4{,}964.2 & 4{,}494.9 & 0.91$\times$ \\
    \rowcolor{gray!8}
    64  & 9{,}312.6 & 8{,}503.4 & 0.91$\times$ \\
    96  & 13{,}085.9 & 10{,}530.6 & 0.80$\times$ \\
    128 & 16{,}825.3 & 13{,}718.1 & 0.82$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
was distorted by several fixable architecture and tooling problems inside rvLLM
rather than by one deep fundamental deficit.
...
\begin{enumerate}
  ...
  \item \textbf{The dominant Hopper FFN seam was wrong.} Direct runner-side phase
        timing on the real $N\!=\!64$ decode bucket showed the \texttt{gateup\_silu}
        to \texttt{down} transition as the main hotspot, and the old `89f`
        CUTLASS gate-aux branch turned out to skip the down-projection entirely.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Gate activation: CUTLASS SM90 auxiliary epilogue
  \item Down-proj: cuBLAS / cublasLt
\end{itemize}

The tooling changed materially as well. The direct benchmark harness was stabilized
(fixed prompts, no intermediate streaming, sync engine, and \texttt{ignore\_eos}),
\texttt{cublasLt} autotune replay was made trustworthy by persisting opaque algorithm
blobs and evicting bad cached plans, and a CUTLASS shared-library + Rust FFI loop
made it possible to iterate on real SM90 collective/epilogue code directly on H100.
...
\begin{itemize}
  \item \textbf{Batch-1 remains behind}: 0.79$\times$ vLLM at $N\!=\!1$.
  \item \textbf{Mid-batch is improved but still behind}: 0.91$\times$ at $N\!=\!32$ and $N\!=\!64$.
  \item \textbf{Large batched decode still trails}: 0.82$\times$ at $N\!=\!128$.
\end{itemize}

This implies that the remaining gap is not one monolithic systems-language tax. The
largest remaining problems are the single-stream decode path, a correct fast Hopper
FFN path, and the last slice of high-end batched efficiency.
...
\begin{enumerate}
  ...
  \item \textbf{Hopper FFN investigation}: the main $N\!=\!64$ batched hotspot is still
        the FFN seam, and the old `89f` gate-aux fast path is no longer considered
        valid because it skipped the down-projection.
\end{enumerate}

The direct-engine table shows that these mechanisms materially improved rvLLM,
but they are not yet sufficient to beat vLLM on the clean current benchmark set.
...
\begin{itemize}
  ...
  \item rvLLM still trails stock vLLM on direct engine at $N\!=\!1$,
        and remains behind at $N\!=\!128$.
...
beat stock vLLM on at least one real batched direct-engine workload on a current
H100 apples-to-apples benchmark: 0.91$\times$ at $N\!=\!64$. The same benchmark
also makes the remaining weaknesses explicit: single-stream decode is still only
0.79$\times$ vLLM at $N\!=\!1$, and large-batch decode is 0.82$\times$ at $N\!=\!128$.

The most important result of this revision is that the public benchmark story now
matches the reproducible code. Correcting the batch-1 default path selection,
expressing the batched hybrid GEMM policy explicitly, and debugging the dominant
Hopper FFN seam removed an invalid fast-path claim and left a clearer target for
future work.

The current tree also demonstrates a practical development approach for this class
of system: explicit GPU memory sizing, no-fallback kernel validation, stable
direct-engine benchmarking, trustworthy autotune replay, and a CUTLASS shared-library
iteration loop, all inside a single Rust workspace.
