...
80\,GB running Qwen2.5-7B-Instruct in FP8~E4M3 with greedy decoding and a
single CUDA-graph replay per step, rvLLM~v3 reaches
\textbf{40{,}331 tokens/second at $N\!=\!512$}
(decode + final RMSnorm + LM head + argmax). At matched batch $N\!=\!128$,
v3 measures \textbf{20{,}841~tok/s} vs vLLM~0.19's
\textbf{19{,}399~tok/s} on the same box with the same model
(\textbf{1.07}$\boldsymbol\times$), but the larger story is that v3's FP8
E4M3 KV cache halves KV memory from 3.6\,GB to 1.8\,GB across 28 layers,
enabling the $N\!=\!256$ and $N\!=\!512$ batches that vLLM's default
f16 KV cannot fit on one 80\,GB card. At those batches v3 reaches
31{,}178 and 40{,}331~tok/s respectively, \textbf{1.61}$\boldsymbol\times$
and \textbf{2.08}$\boldsymbol\times$ vLLM's $N\!=\!128$ throughput.
The earlier v2 stack peaked at 19{,}287 tok/s at $N\!=\!128$
(SHA \texttt{eb9e247fd}) and is kept in-tree as an archive.
...
\textbf{40{,}331~tok/s at $N\!=\!512$} and 20{,}841~tok/s at matched
$N\!=\!128$ on a single H100 under a bench that re-uploads metadata each
step and runs a 16-step faux-prefill (fair-comparison configuration,
Section~\ref{sec:results}). The previous v2 stack (archived at commit
\texttt{eb9e247fd}, April~16, 2026) reached 19{,}287~tok/s at $N\!=\!128$;
vLLM~0.19 V1 on the same box measures 19{,}399~tok/s at the same batch
but cannot run at $N\!=\!256$ or $N\!=\!512$ within the 80\,GB
envelope (its f16 KV cache exhausts HBM). v3's primary levers are: (i)
...
\subsection{v3 throughput by batch size (FP8~E4M3 KV, ITERS=512)}

\begin{tabular}{rrrr}
\toprule
$\boldsymbol{N}$ & \textbf{tok/s} & \textbf{ms/step} & \textbf{vs vLLM $N\!=\!128$} \\
\midrule
128 & 20{,}841 & 6.14 & $+$7.4\% \\
256 & 31{,}178 & 8.21 & $+$60.7\% \\
512 & \textbf{40{,}331} & \textbf{12.70} & $+$\textbf{108\%} \\
\bottomrule
\end{tabular}

\subsection{Head-to-head at $N\!=\!128$ (matched batch)}

\begin{tabular}{lrr}
\toprule
\textbf{Engine} & \textbf{tok/s} & \textbf{ratio to v3} \\
\midrule
vLLM 0.19 V1 (FP8, FlashInfer, CUDA graphs) & 19{,}399 & 0.931$\times$ \\
rvllm-v2 (archive, \texttt{eb9e247fd}) & 19{,}287 & 0.925$\times$ \\
\textbf{rvllm-v3 (FP8 KV + fair bench)} & \textbf{20{,}841} & \textbf{1.00}$\boldsymbol\times$ \\
\bottomrule
\end{tabular}

\paragraph{Batches above $N\!=\!128$.}
vLLM could not be run at $N\!=\!256$ or $N\!=\!512$ on the same
80\,GB card in its default configuration --- vLLM's KV cache is f16
(2~bytes/element), which with our 28-layer Qwen2.5-7B page configuration
requires enough HBM that scaling past $N\!\approx\!128$ triggers OOM in
the KV allocator. rvLLM~v3 uses FP8~E4M3 KV (1~byte/element), halving
each layer's KV allocation. At $N\!=\!256$ and $N\!=\!512$ the GEMMs
approach HBM saturation --- rather than being dispatch-bound as at
smaller batches --- and absolute throughput scales near-linearly with
batch until the attention softmax-bandwidth wall.
...
\textbf{20{,}841~tok/s} at $N\!=\!128$. The per-step metadata upload
and faux-prefill are small relative to the 28-layer forward pass and
LM head. v3 at matched $N\!=\!128$ is $1.07\times$ vLLM~0.19 on the
same box; scaling to $N\!=\!256$ and $N\!=\!512$ --- which only v3 can
fit in 80\,GB because of the FP8 E4M3 KV cache --- yields $1.61\times$
and $2.08\times$ respectively, confirming the edge is structural
(fused QKV, cuBLASLt autotune, fused epilogues, FP8 KV memory,
no Python dispatch).
...
\subsection{v3 session progression}

\begin{tabular}{lrrr}
\toprule
\textbf{Step} & \textbf{tok/s} & \textbf{Batch} & $\boldsymbol\Delta$ \\
\midrule
Eager (no graph)                              & 551     & 128 & --- \\
+ CUDA graph capture                          & 14{,}745 & 128 & 27$\times$ \\
+ f16 RoPE tables                             & 15{,}537 & 128 & +5\% \\
+ q/k/v bias load + fused QKV                  & 15{,}985 & 128 & +3\% \\
+ cuBLASLt + \texttt{EPILOGUE\_BIAS} (QKV)    & 17{,}562 & 128 & +10\% \\
+ all 5 linears on cuBLASLt                   & 22{,}496 & 128 & +28\% \\
+ FP8 E4M3 KV + final RMSnorm                 & 20{,}841 & 128 & $-$7\%, but\ldots \\
\ldots FP8 KV unlocks $N\!=\!256$            & 31{,}178 & 256 & +\textbf{50}\% \\
\textbf{\ldots and $N\!=\!512$}              & \textbf{40{,}331} & \textbf{512} & +\textbf{29}\% \\
\bottomrule
\end{tabular}

At matched batch $N\!=\!128$ the FP8 KV migration is a small
regression (FA3 dequant overhead exceeds the HBM bandwidth savings at
short contexts). The value of FP8 KV is the 2$\times$ memory halving
that unlocks the larger batches, where GEMM saturation wins dominate.
...
\textbf{40{,}331~tok/s} at $N\!=\!512$ (and 20{,}841~tok/s at matched $N\!=\!128$) serving Qwen2.5-7B in FP8 on a single H100, with
