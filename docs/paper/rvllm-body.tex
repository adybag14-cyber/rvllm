...
152.9~tok/s at $N\!=\!1$ (\textbf{0.91$\times$ vLLM}),
9{,}589.1~tok/s at $N\!=\!64$ (\textbf{1.04$\times$ vLLM}), and
15{,}479.9~tok/s at $N\!=\!128$ (\textbf{0.92$\times$ vLLM}). The most important
changes in this revision are architectural and tooling-heavy rather than cosmetic:
...
cublasLt, and the dominant Hopper FFN seam now uses a CUTLASS SM90
auxiliary-epilogue gate path that computes $\mathrm{SiLU}(\mathrm{gate}) \cdot \mathrm{up}$
without the older bad materialization boundary. rvLLM is open-source under the
Apache-2.0 license.
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}rrrr@{}}
    ...
    \midrule
    1   & 167.6 & 152.9 & 0.91$\times$ \\
    \rowcolor{gray!8}
    \textbf{64}  & \textbf{9{,}256.4} & \textbf{9{,}589.1} & \textbf{1.04$\times$} \\
    128 & 16{,}904.8 & 15{,}479.9 & 0.92$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{Batch-1 default path selection was wrong.} Normal $T\!=\!1$ decode
        ...
        Fixing that lifted the current normal batch-1 path to 152.9~tok/s.
  ...
  \item \textbf{The dominant Hopper FFN seam was wrong.} Direct runner-side phase
        ...
        to \texttt{down} transition as the main hotspot, and the old path still left
        the wrong materialization boundary in place.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Batch-1 remains behind}: 0.91$\times$ vLLM at $N\!=\!1$.
  \item \textbf{The main batched regime can now win}: 1.04$\times$ at $N\!=\!64$.
  \item \textbf{Large batched decode still trails}: 0.92$\times$ at $N\!=\!128$.
\end{itemize}

This implies that the remaining gap is not one monolithic systems-language tax. The
normal batched stack is now competitive enough to beat vLLM on at least one real
public workload; the largest remaining problems are the single-stream decode path
and the last slice of high-end batched efficiency.
...
\begin{enumerate}
  ...
  \item \textbf{Hopper FFN gate epilogue}: the main $N\!=\!64$ batched hotspot now
        uses a CUTLASS SM90 auxiliary-epilogue path instead of the older bad FFN seam.
\end{enumerate}

The direct-engine table shows that these mechanisms are sufficient to make rvLLM
beat vLLM at $N\!=\!64$, but not yet enough to win at $N\!=\!1$ or $N\!=\!128$.
...
H100 apples-to-apples benchmark: 1.04$\times$ at $N\!=\!64$. The same benchmark
also makes the remaining weaknesses explicit: single-stream decode is still only
0.91$\times$ vLLM at $N\!=\!1$, and large-batch decode is 0.92$\times$ at $N\!=\!128$.
...
selection, expressing the batched hybrid GEMM policy explicitly, and fixing the
dominant Hopper FFN seam moved the comparison from a uniform ``rvLLM is slower''
story to a split picture: batched decode can win, single-stream decode still does not.
